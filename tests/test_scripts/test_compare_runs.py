...
from scripts.compare_runs import (
    Severity, Confidence, DiscrepancyType, Discrepancy, ComparisonReport,
    find_versions, get_proviral_csv_files, read_csv_file, compare_csv_contents,
    compare_proviral_files, compare_runs, main,
    _determine_row_difference_severity, _determine_row_difference_confidence
)
...
class TestDiscrepancy:
    ...
    def test_discrepancy_creation(self):
        ...
        discrepancy = Discrepancy(
            ...
            Confidence.HIGH,
            "Test discrepancy",
            location={"file": "test.csv", "version": "version_7.15", "run": "both", "row": 2},
...
class TestCSVComparison:
    ...
    def test_compare_different_row_count(self, tmp_path):
        ...
        row_count_discrepancy = next(
            (d for d in discrepancies if d.type == DiscrepancyType.ROW_COUNT_DIFFERENCE), None
        )
    ...
    def test_compare_different_headers(self, tmp_path):
        ...
        header_discrepancy = next(
            (d for d in discrepancies if d.type == DiscrepancyType.HEADER_DIFFERENCE), None
        )
    ...
    def test_compare_one_empty_file(self, tmp_path):
        ...
        error_discrepancy = next(
            (d for d in discrepancies if d.type == DiscrepancyType.FILE_READ_ERROR), None
        )
...
class TestFullComparison:
    ...
    def test_compare_successful_run(self, tmp_path):
        ...
        (run2_dir / "Results" / "version_7.15" / "proviral" / "test.csv").write_text("header\ndifferent\n")
    ...
    def test_compare_with_comparison_error(self, tmp_path):
        ...
        with patch('scripts.compare_runs.compare_proviral_files', side_effect=Exception("Test error")):
            report = compare_runs(run1_dir, run2_dir)
        
        # Should have comparison error discrepancy
        assert "comparison_error" in report.results["version_7.15"]
        discrepancy = report.results["version_7.15"]["comparison_error"]["discrepancies"][0]
...
class TestMainFunction:
    ...
    def test_main_with_output_file(self, tmp_path):
        ...
        with patch('sys.argv', ['compare_runs.py', str(run1_dir), str(run2_dir), '-o', str(output_file)]):
            main()
    ...
    def test_main_with_compact_output(self, tmp_path, capsys):
        ...
        with patch('sys.argv', ['compare_runs.py', str(run1_dir), str(run2_dir), '--compact']):
            main()
    ...
    def test_main_keyboard_interrupt(self, tmp_path):
        ...
        with patch('scripts.compare_runs.compare_runs', side_effect=KeyboardInterrupt()):
            with patch('sys.argv', ['compare_runs.py', str(run1_dir), str(run2_dir)]):
                with pytest.raises(SystemExit) as exc_info:
                    main()
                assert exc_info.value.code == 1
