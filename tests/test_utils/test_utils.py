...
def test_get_softclip_start():
    ...
    # Generate alignment with mappy instead of loading SAM file
    cigar_hits = utils.align(utils.mod_hxb2, query_sequence, 'test_softclip', outdir=example)
    
    # Get soft-clipped region using the new API
    softclipped_seq = utils.get_softclipped_region(query_sequence, cigar_hits)
    valid_softclipped_seq = utils.load_yaml(example / 'valid' / 'valid_softclipped_seq.yaml')
